...
export default class AppointmentsActivity extends Component {
  ...
  _retriveData = async () => {
    try {
      ...
      fetch( url.base_url + "/fetchAppointmentResponse", {
        method: 'POST',
        headers: {
        ...
        if(responseJson)
        {
          ...
          console.log(e)
           ...
           this.setState({data: [{id : responseJson[0].app_ID,
            ...
            description: responseJson[0].Name + ' has ' + responseJson[0].status + ' your Appointmnet Request of date ' + responseJson[0].Date.slice(0,10) ,  }]})
               ...
               for (i=1; i < Object.keys(responseJson).length ; i++)
                {
                      ...
                      description:responseJson[i].Name + ' has ' + responseJson[0].status + ' your Appointmnet Request of date ' + responseJson[i].Date.slice(0,10) ,
                      })
                }
  ...
  renderItem = ({ item }) => (
    <ListItem
      ...
      onPress={ () => this.props.navigation.navigate('AppointmentResponseView',{
        app_id: item.id,
      })}
      bottomDivider
    />
