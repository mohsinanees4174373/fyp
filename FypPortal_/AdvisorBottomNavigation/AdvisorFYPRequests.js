...
export default class AdvisorFypRequestActivity extends Component {
  ...
  _retriveData = async () => {
    try {
      ...
      fetch(url.base_url + '/fetchFypRequests', {
        method: 'POST',
        headers: {
        ...
        .then((responseJson) => {
          if (responseJson[0]) {
            ...
            RNFetchBlob.fetch(
              'GET',
              url.base_url + '/getfile?path=' + responseJson[0].profile_pic,
              {},
            )
              .then((res) => {
            ...
            this.setState({
              data: [
                {
                  id: responseJson[0].fyp_ID,
                  avatar_url: this.state.uri,
                  name: responseJson[0].Name,
                  description:
                    'You have received appointment Request from ' +
                    responseJson[0].Name,
                },
              ],
            });
            ...
            for (i = 1; i < Object.keys(responseJson).length; i++) {
              ...
              this.state.data.push({
                ...
                description:
                  'You have received FYP request from ' + responseJson[i].Name,
              });
            }
          } else {
            this.setState({data: []});
  ...
  renderItem = ({item}) => (
    <ListItem
      ...
      leftAvatar={
        item.avatar_url ? (
          <Image
            source={{uri: `data:jpeg;base64,${item.avatar_url}`}}
      ...
      onPress={() =>
        this.props.navigation.navigate('FypRequestView', {
          req_id: item.id,
        })
      }
      bottomDivider
